...
function renderSoftware() {
  ...
  const items = Array.isArray(siteData.software) ? siteData.software : [siteData.software];
    ...
    card.innerHTML = `
      ...
      <div class="software-footer">
        <div class="software-stats">
          ${sw.stats.map(s => s.label
            ? `<span class="stat"><strong>${s.value}</strong> ${s.label}</span>`
            : `<span class="stat"><strong>${s.value}</strong></span>`
          ).join('')}
        </div>
