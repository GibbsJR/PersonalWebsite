...
// ============================================
// SOFTWARE SHOWCASE RENDERING
// ============================================

function renderSoftware() {
  const container = document.getElementById('software-showcase');
  if (!container || typeof siteData === 'undefined' || !siteData.software) return;

  const sw = siteData.software;
  ...
  card.innerHTML = `
    <div class="software-header">
      <div class="software-title-group">
        <h3>${sw.name}</h3>
        <span class="software-tagline">${sw.tagline}</span>
      </div>
      <a href="${sw.links.github}" target="_blank" rel="noopener" class="software-github-link">
    ...
    <p class="software-description">${sw.description}</p>
    <div class="software-highlights">
      <h4>Key Features</h4>
      <ul>
        ${sw.highlights.map(h => `<li>${h}</li>`).join('')}
      </ul>
    </div>
    <div class="software-footer">
      <div class="software-stats">
        <span class="stat"><strong>${sw.stats.tests}</strong> tests</span>
        <span class="stat"><strong>${sw.stats.notebooks}</strong> tutorials</span>
        <span class="stat"><strong>${sw.stats.language}</strong></span>
      </div>
